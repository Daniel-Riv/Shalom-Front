import { CarouselConten } from './CarouselConten';


export const CarouselC = () => {

    //const [sliceIndex, setSliceIndex] = useState(1);
    const { imgOne, imgTwo, imgThree } = CarouselConten();

    /* const nextSlice =()=>{
        if (sliceIndex !== CarouselConten().length) {
            setSliceIndex(sliceIndex + 1);    
        }else if (sliceIndex === CarouselConten().length) {
            setSliceIndex(1);
        }
    }
    ...
    const prevSlice =()=>{
        if (sliceIndex !== 1) {
            ...
            setSliceIndex(CarouselConten().length);
        }
    }
            ...
            <div id="default-carousel" className="relative" data-carousel="static">
                ...
                <div className="relative h-56 overflow-hidden rounded-lg md:h-96">
                    ...
                    <div className="duration-700 ease-in-out absolute inset-0 transition-all transform translate-x-0 z-20" data-carousel-item="">
                        ...
                        <img src={imgThree} className="absolute block w-full -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2" alt="..."/>
                    </div>
                    {/* <!-- Item 2 --> */}
                    <div className="duration-700 ease-in-out absolute inset-0 transition-all transform translate-x-full z-10" data-carousel-item="">
                        <img src={imgTwo} className="absolute block w-full -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2" alt="..."/>
                    </div>
                    {/* <!-- Item 3 --> */}
                    <div className="duration-700 ease-in-out absolute inset-0 transition-all transform -translate-x-full z-10" data-carousel-item="">
                        <img src={imgOne} className="absolute block w-full -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2" alt="..."/>
                    </div>
                </div>
                {/* <!-- Slider indicators --> */}
